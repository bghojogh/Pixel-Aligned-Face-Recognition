function [ egnFace , egnValSort , meanFace , varargout] = PCA( trnFace  , varargin) % TrnFace, AvFace: each row is an observation (to be consistant with MATLAB). EgnFace: each column is an eigenface. EgnValSort: row vecotr

egnPow = 0.99; w = 1; test={};
...
trnFaceCnt = double(trnFace) - repmat( meanFace , trnNum , 1 );
